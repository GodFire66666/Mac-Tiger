...
gpt_questions = get_question_and_answers(
    "One full pitcher of beer with an elephant's trunk in it."
)
gpt_questions
...
"""
    ...
    def generate_response(self, user_prompt, generated_image, previous_prompts=None):
        filtered_questions, VS_result = self.calculate_VS_score(
            user_prompt, generated_image
        )
...
filtered_questions, VS_result, reviewer_evaluation = reviewer.generate_response(
    prompt, image
)
reviewer_evaluation
...
challenger_response = challenger.generate_response(
    prompt, image, filtered_questions, VS_result, reviewer_evaluation
)
...
refiner = Refiner()
refiner_response = refiner.generate_response(
    prompt,
    image,
    filtered_questions,
    VS_result,
...
best_image = choose_best_image(image, new_image, VS_result, new_VS_result).save(
    "best_image.jpg"
)
